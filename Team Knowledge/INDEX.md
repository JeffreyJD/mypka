...
**2026-07-18 — Parallel dispatch + "next free number" is a race, not just a lookup (Hawkeye coordination rule)**
When two agents are dispatched in parallel and each independently confirms the "next free" sequence number for a numbered artifact type (GL-NNN, SOP-NNN, WS-NNN) by listing the relevant directory, both can see the same free number and both can write to it before either commits — the confirm-then-write gap is where the race lives, not the confirm step itself. Evidence: during the 2026-07-18 WS-004 Tier 2 retro, Bastion and Vex were dispatched in parallel, each correctly listed `Team Knowledge/Guidelines/` and correctly saw GL-014 as free, and both wrote a different Guideline to `GL-014-*.md` before either agent's session closed. Caught only because Hawkeye reviewed each agent's actual file diff directly before closing the batch (rather than trusting self-reports) and noticed two files sharing a number. Fix applied ad hoc: renumbered the second file to GL-015 and patched all cross-references. **Resolved 2026-07-18 as a written rule:** Jeff chose mandatory pre-commit collision check over serializing numbered-artifact creation ([[tsk-2026-07-18-014-numbered-artifact-collision-rule]]); the mechanism is [[GL-016-numbered-artifact-collision-check]] — re-confirm the number immediately before writing, plus a required batch-wide check across every new numbered artifact before any shared commit. Wired into [[GL-001-file-naming-conventions]] §6, [[SOP-010-create-task]] §2, the "how to add" sections of the Guidelines/SOPs/Workstreams indexes, and [[WS-004-team-retro-and-self-improvement-loop]] Tier 2 Step 5.

...
- **OneNote `.one` / `.onetoc2` files — binary, stub-only.** These are proprietary binary formats; the LLM cannot extract content from them by reading bytes. When one is encountered during the inventory walk (§3), create a stub `Document` note (`Team Knowledge/Templates/document.md`) pointing at the original file's path and noting the format, and flag it in the plan (§4) that the user needs to export the notebook to PDF/DOCX/HTML from OneNote itself before real content import is possible. Never attempt a text-mode read on these — it silently produces binary garbage in the note body instead of a clean failure.
- **Office lock files (`~$*.docx`, `~$*.xlsx`, `~$*.pptx`, etc.) — always safe to discard.** These are transient artifacts Office creates while a file is open, not content of their own. Exclude them from the inventory (§3) entirely — no stub, no anomaly flag, no per-file session-log line. A single rollup count ("N Office lock files skipped") in the session-log's counts section is sufficient.
- **Slug-prefix collision-avoidance convention (reusable across bulk imports).** When an import walks several source subfolders into the same destination entity folder (e.g. many standalone Documents pulled from unrelated subfolders of one backup drive), prefix each generated slug with the source subfolder's own slug — `<source-folder-slug>-<original-slug>.md` — rather than leaning on the `-from-<source>` rename-on-conflict suffix from Step 2's conflict policy. This avoids collisions up front instead of discovering them at write time, and keeps files from the same source subfolder grouped when the destination folder is listed flat. Apply this proactively whenever a single import spans multiple source subfolders landing in one destination folder, not just as a conflict-policy fallback.

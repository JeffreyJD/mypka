...
| SOP-023 | [[SOP-023-triage-inbox]] | Radar | Take one inbox (`inbox_id`) from full to clean without missing anything. Five phases: scope+load per-inbox state (Phase 0), fetch window since last run with message-id dedup (Phase 1), classify per [[GL-018-inbox-triage-and-classification]] (Phase 2), dispatch by disposition with propose-then-confirm (Phase 3), report+persist state+journal (Phase 4). Dry-run default. LLM-agnostic, two-path notes. References [[GL-018-inbox-triage-and-classification]], [[SOP-010-create-task]], [[GL-012-confirm-dispatch-not-just-narrate-it]]. |

*Reserved (genuinely open for future agents):* SOP-024 onward. Do not back-fill below SOP-023 without coordinating across the team.

...
- [ ] Jeff provisions a new `HEALTHCHECKS_WEEKLY_REPORT_URL` dead-man's switch (same pattern as the existing autopsy/fidelity checks) before first live fire. **Still outstanding — flagging again here.**
- [ ] A new, separate `ANTHROPIC_WEEKLY_REPORT_API_KEY` needs provisioning on the VPS (never the existing unused `ANTHROPIC_API_KEY` placeholder). **Not yet done.**

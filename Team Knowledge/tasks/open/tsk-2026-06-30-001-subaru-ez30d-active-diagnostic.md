...
### HIGH — this week
- [ ] **FIRST: check for mated GREEN test-mode connectors under driver's-side dash (near steering column).** If connected → car is in dealer test mode = constant CEL blink + fans/fuel pump/solenoid cycling. Unplug them (leave hanging open), key cycle, confirm blink stops. Likely left connected during 6/15 fuel pump job (standard pump-priming technique). If this is it: re-baseline EVERYTHING — re-scan DTCs, re-check EVAP readiness, and re-evaluate the O2 code batches and fan observations, all of which may be test-mode artifacts

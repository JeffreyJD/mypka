...
- 2026-07-20 (sparky) — **802.11r-disable experiment: approved by Jeff, design decision drafted, not yet executed.** Jeff approved the controlled 24–48h experiment proposed above (disable 802.11r Fast Roaming on "Downton Abbey" only, watch for recurrence). Per Sparky's operating discipline, a written design decision was drafted before any controller change: [[2026-07-20-design-decision-disable-80211r-downton-abbey]] — covers exact change scope, UI location (flagged as needing live verification against the actual controller), 24–48h duration with re-check dates, success/failure signal tied back to the 2026-07-20 flap baseline, mandatory rollback (re-enable regardless of outcome), and the reassociation-delay trade-off during the window. **Status: approved, not executed** — the actual toggle flip requires a live browser session against `https://192.168.1.6`, which this Sparky dispatch does not have tools for; execution is a separate, later step (by Jeff/Hawkeye live, or a future dispatch with browser/API access). [[network-design]] Pending item #8 and Change log updated to match. This task remains open pending that execution and the post-window re-check/analysis.
- 2026-07-20 (sparky) — **802.11r-disable experiment: EXECUTED.** Toggle flipped live at **2026-07-20T23:01:10Z UTC** (~7:01 PM local) via `https://192.168.1.6` → Settings → WiFi → "Downton Abbey" → Advanced (Manual mode) → Roaming Assistance → unchecked "Fast Roaming (802.11r)" → Apply Changes confirmed (pending-change bar cleared). "Handoff Suggestions (802.11v)" was already unchecked, not part of this change. Only "Downton Abbey" touched; Downton Abbey-IOT and Downton Abbey-Guest not opened or modified. **Re-check windows: 24h = 2026-07-21T23:01Z, 48h = 2026-07-22T23:01Z.** Separately, Jeff self-reported a fresh Wi-Fi drop in the few minutes just before the toggle flip (~22:55–23:00Z, approximate/unconfirmed timing) — recorded as one more pre-experiment baseline data point in [[jeff-laptop]] § Incidents, not conflated with the disabled-window results. Full execution record: [[2026-07-20-design-decision-disable-80211r-downton-abbey]] Change log. [[network-design]] SSID register, Pending item #8, and Change log updated; [[uck-g2-plus]] updated. **This task remains open** pending the 24h/48h re-check Events-log pull and comparison against the 2026-07-20 baseline (per this Deliverable §4 success/failure signal).

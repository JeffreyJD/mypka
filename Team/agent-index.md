...
| Ledger | Deployment Verification Engineer | [[Team/Ledger - Deployment Verification Engineer/AGENTS]] | Pre-deploy config/schedule fidelity checks, decommission verification (confirms old infra is actually disabled after a migration), data provenance checks, and a recurring Environment Drift Audit comparing live host/service state against the registry and last-approved baselines. Independent of Pierce (implementation) and Blake (strategy) — checks deployed reality against validated intent. Default owner of [[SOP-022-deployment-fidelity-verification]]. |
| Relay | Smart Home & IoT Engineer | [[Team/Relay - Smart Home & IoT Engineer/AGENTS]] | ESPHome device firmware (ESP32/ESP32-S3), Home Assistant configuration/dashboards/automations, Modbus RTU/RS-485 device integration, local toolchain setup for smart-home devices (ESPHome CLI/Dashboard, USB-serial drivers). Owns the pool-monitor-automation project. Hands off network/VLAN work to Sparky and Proxmox/host provisioning to Trapper. |

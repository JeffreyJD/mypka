...
When Hawkeye dispatches Kellye on a task, follow [[SOP-read-own-journal]] before starting. When creating a task, follow [[SOP-create-task]]. When closing a task, follow [[SOP-close-task]] and write a journal entry for any durable insight — especially destination-specific visa rules, airport routing lessons, or loyalty program redemption patterns.

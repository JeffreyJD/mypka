...
**`/watch` output does not persist on its own, per [[GL-011-capture-watch-summaries-before-session-end]].** Any `/watch` call producing a substantive answer gets captured into a session log or journal entry before the session ends — the plugin deletes its working directory at close, and future sessions don't read old conversation transcripts. Not conditional on the user saying "remember this."
